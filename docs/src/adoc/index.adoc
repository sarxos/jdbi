...
[NOTE]
Jdbi's default setting is to convert Map keys to lowercase. This behavior can be
...
link:{jdbidocs}/core/mapper/GenericMapMapper.html[GenericMapMapper^],
which provides the same feature but allows value types other than `Object`
as long as a suitable ColumnMapper is registered:

[source,java]
----
@SqlQuery("select 1.0 as LOW, 2.0 as MEDIUM, 3.0 as HIGH")
@RegisterRowMapper(GenericMapMapper.class)
@SingleValue
